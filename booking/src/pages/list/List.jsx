...
// if(city ==='city'){
  ...
  if(!destination && type){
     url = `http://localhost:8800/api/hotels?type=${type}&min=${min || 0 }&max=${max || 999}`
  }
  else if(destination && !type)
  {
   url =`http://localhost:8800/api/hotels?city=${destination}&min=${min || 0 }&max=${max || 999}`
  }
  else if(destination && type){
    url =`http://localhost:8800/api/hotels?city=${destination}&type=${type}&min=${min || 0 }&max=${max || 999}`;
  }
  ...
  const isNameValid =(cityName)=>{
    const citiesArr = ['hanoi','danang','dalat','vungtau','hochiminh','hcm']
  ...
  const handleSearch = () => {
    if(isNameValid(destination))
    {
      ...
      return alert("we currently not don't have that location available on our website yet.Please choose another location or come back to homepage to see the destinations that we currently have")
        ...
        <div className="listWrapper">
          <div className="listSearchContainer">
            <div className="listSearch">
              ...
              <label for="room-type" style={{"fontSize":"12px"}}>Type: </label>
